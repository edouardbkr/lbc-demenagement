/**
 * service-fond.jsx — contenu de fond des pages de service, et le composant qui l'affiche.
 *
 * POURQUOI UN MECANISME PARTAGE
 * Contrairement aux villes, aux quartiers et aux trajets, chaque page de service a
 * son propre composant : il n'existe pas de gabarit commun ou brancher le contenu.
 * Plutot que de modifier chaque fichier a la main, ce fichier porte a la fois les
 * donnees et le composant <FondPage cle="..." />, qu'une page appelle en une ligne.
 *
 * REGLE D'ECRITURE
 * Ces pages ne parlent pas de villes. Elles repondent a une intention de recherche
 * differente : comment faire, combien ca coute, dans quel ordre. Si une section
 * pouvait vivre sur une page ville, elle est mal placee.
 *
 * CONTROLE  node audit-complet.js service
 */
...
const SERVICE_FOND = {
  ...
  "checklist": {
    ...
    sections: [
      ...
      { t: "À huit semaines : le devis et le tri", d: "Faites établir un devis sur inventaire détaillé, pas sur une surface annoncée au téléphone. Un professionnel qui chiffre sans connaître l'accès, l'étage et le volume réel se trompe, et l'écart se règle le jour J à votre charge. Profitez de ce moment pour commencer le tri : ce que vous ne voulez plus ne doit pas être transporté, parce que le transport se paie au volume. Ce qui part en don ou en vente demande du temps, c'est maintenant ou jamais." },
      { t: "À six semaines : les administrations", d: "Le changement d'adresse concerne plus d'organismes qu'on ne croit : employeur, banque, assurances, mutuelle, impôts, caisse d'allocations, énergie, internet, école. Certaines démarches ont un délai contraint, notamment la résiliation ou le transfert des contrats d'énergie et d'internet, dont le raccordement peut prendre plusieurs semaines. Une arrivée sans électricité ni connexion est une arrivée gâchée pour rien." },
      ...
      { t: "À deux semaines : les autorisations et l'accès", d: "Si votre rue ou celle d'arrivée demande une autorisation de stationnement, elle se dépose maintenant. Prévenez le syndic ou le gardien aux deux adresses, réservez l'ascenseur si le règlement l'impose, et récupérez les codes et les badges. Vérifiez la largeur des portes, la présence d'un monte-charge, et la possibilité de stationner un véhicule long. Ces vérifications prennent une heure et évitent la moitié des mauvaises surprises." },
      { t: "La dernière semaine, et le jour J", d: "Vider le réfrigérateur et le dégivrer, débrancher et sécuriser la machine à laver, relever les compteurs, préparer un carton d'arrivée avec ce dont vous aurez besoin le premier soir : papier toilette, lampe, chargeurs, draps, de quoi manger. Le jour même, faites le tour complet du logement pièce par pièce, cave et balcon compris, avant que le camion ne ferme. C'est le quart d'heure le plus rentable de tout le déménagement." }
    ],
    erreurs: [
      { t: "Réserver la date en dernier", d: "Les samedis et fins de mois partent des mois à l'avance en haute saison. C'est la première chose à bloquer, avant même d'avoir tout organisé." },
      { t: "Oublier le délai de raccordement", d: "Internet et parfois l'électricité demandent plusieurs semaines. Une arrivée sans connexion se prépare, ou se subit." },
      { t: "Étiqueter par contenu", d: "L'étiquette doit dire où poser le carton, pas ce qu'il contient. « Cuisine » fait gagner une heure, « vaisselle » ne sert à rien." }
    ]
  },
  ...
  "formules": {
    ...
    sections: [
      { t: "Ce que le prix recouvre réellement", d: "Un devis de déménagement additionne quatre choses : le volume, la distance, l'accès aux deux adresses, et le niveau de prestation. Le volume et la distance sont les seuls que vous puissiez estimer vous-même. L'accès, c'est-à-dire l'étage, la présence d'un ascenseur adapté, la distance de portage et la possibilité de stationner, pèse souvent autant que le volume et ne se devine pas à distance. C'est la raison pour laquelle le devis vous demande, aux deux adresses, l'étage, l'ascenseur et sa taille, la distance de portage et la possibilité de stationner. Une visio de dix minutes lève ce qui resterait en doute. Le prix annoncé ensuite ne bouge plus." },
      { t: "Quand la formule économique suffit", d: "Elle convient si vous avez du temps devant vous, un volume modéré, et un accès simple aux deux adresses. Vous emballez, nous transportons. C'est le bon choix pour un studio ou un deux-pièces sans mobilier fragile, quand vous pouvez consacrer plusieurs soirées à préparer. Le piège est de la choisir pour le prix alors qu'on n'aura pas le temps : un emballage bâclé coûte plus cher en casse que la différence entre deux formules." },
      { t: "Quand il faut passer à la formule complète", d: "Dès qu'il y a du démontage-remontage, du mobilier fragile ou de valeur, un étage sans ascenseur, ou un délai court. Le démontage d'un lit, d'une armoire ou d'une cuisine demande des outils et de la méthode, et un meuble démonté sans repérage ne se remonte pas toujours. Cette formule inclut la protection du mobilier, ce qui n'est pas un détail sur un accès difficile. C'est celle que nous recommandons dans la majorité des cas, et nous le disons franchement même quand ce n'est pas ce que le client espère entendre." },
      { t: "Ce que change l'assurance", d: "Dans nos trois formules, vos meubles sont assurés du chargement à la livraison, jusqu'à un plafond par objet indiqué sur votre devis. Ce point mérite d'être vérifié chez tout prestataire que vous comparez : certaines offres à bas prix couvrent le camion et pas le contenu, ou plafonnent à un niveau qui ne couvre rien. Un devis moins cher sans assurance équivalente n'est pas un devis moins cher, c'est un devis différent." },
      { t: "Comment comparer deux devis honnêtement", d: "Mettez côte à côte quatre lignes : le volume retenu, le nombre d'équipiers, ce qui est inclus en démontage et en emballage, et le plafond d'assurance par objet. Si l'un des devis a été fait sans visite, il est structurellement moins fiable que l'autre, quel que soit son montant. Et vérifiez si le prix est ferme ou révisable le jour J : c'est là que se creusent la plupart des écarts entre le devis signé et la facture finale." }
    ],
    erreurs: [
      { t: "Choisir sur le seul montant affiché", d: "Deux devis ne couvrent presque jamais le même périmètre. Le moins cher est souvent celui qui inclut le moins, et l'écart se règle le jour J." },
      { t: "Prendre l'économique par optimisme", d: "Si vous n'aurez pas le temps d'emballer correctement, la formule au-dessus coûte moins que la casse et les heures supplémentaires." },
      { t: "Accepter un devis sans visite", d: "L'accès pèse autant que le volume et ne se devine pas. Un devis téléphonique est une estimation, pas un engagement." }
    ]
  },
  ...
  "archivage": {
    ...
    sections: [
      ...
      { t: "La destruction, quand elle est possible", d: "Ce qui a dépassé sa durée de conservation obligatoire peut être détruit, et le faire libère du volume et donc du coût. Cette destruction doit être documentée quand les documents sont sensibles : on établit un certificat mentionnant la nature et le volume détruits. Nous vous indiquons ce que nous pouvons prendre en charge et ce qui relève d'un prestataire spécialisé, plutôt que de vous laisser croire que tout se règle avec une benne." }
    ],
    erreurs: [
      { t: "Transférer sans trier", d: "Stocker des documents dont la durée de conservation est dépassée coûte chaque mois pour rien. Le tri se fait avant le transfert, pas après." },
      { t: "Mélanger archives et mobilier", d: "Les contenants doivent être scellés, numérotés et suivis. Un carton d'archives dans un camion de déménagement classique n'a aucune traçabilité." },
      { t: "Se passer d'inventaire", d: "Un carton sans référence est un carton introuvable. L'inventaire au contenant est ce qui rend le stockage utile plutôt que subi." }
    ]
  },
  ...
  "urgence": {
    ...
    sections: [
      { t: "Ce qui est vraiment bloquant en urgence", d: "Trois choses seulement peuvent empêcher un déménagement rapide : la disponibilité d'une équipe et d'un véhicule adaptés, l'accès aux deux adresses, et l'autorisation de stationnement quand la rue l'impose. Le reste, tri, cartons, administratif, peut se traiter en parallèle ou après. Lors du premier appel, nous vérifions ces trois points avant toute chose, parce que ce sont eux qui déterminent si la date est tenable. Nous préférons vous dire non tout de suite plutôt que d'accepter et de vous laisser sans solution." },
      { t: "L'autorisation de voirie en délai court", d: "C'est le point le plus contraint. Certaines communes instruisent en quelques jours, d'autres demandent davantage, et sans emplacement réservé le camion n'a nulle part où se mettre. Nous engageons la demande dans l'heure qui suit votre accord, et nous identifions immédiatement une solution de repli quand le délai ne peut pas être tenu : report sur une rue voisine, créneau très matinal, ou portage plus long chiffré à l'avance." },
      { t: "Emballer vite sans emballer mal", d: "En urgence, la tentation est de tout jeter dans des cartons. C'est ce qui produit la casse. Notre équipe emballe avec vous, en commençant par ce qui est fragile et par ce qui est vital à l'arrivée, et en laissant pour la fin ce qui supporte un traitement rapide. L'étiquetage est réduit au strict nécessaire, la pièce de destination, mais il est fait. Un carton non étiqueté en urgence est un carton qu'on ouvrira trois fois." },
      { t: "Le garde-meuble, souvent la vraie solution", d: "Beaucoup d'urgences ne sont pas des urgences de déménagement mais des urgences de libération : il faut vider un logement à une date, sans que le logement d'arrivée soit prêt. Dans ce cas, la bonne réponse n'est pas de tout précipiter mais de charger, inventorier et stocker, puis de livrer à froid quand vous êtes prêt. Cela coûte moins qu'un déménagement bâclé et supprime la pression." },
      { t: "Les situations que nous traitons le plus souvent", d: "Expulsion ou fin de bail non anticipée, mutation professionnelle annoncée tardivement, séparation, sinistre rendant le logement inhabitable, vente conclue plus vite que prévu. Ce sont des moments difficiles, et nous les traitons sans commentaire ni jugement. Ce que nous demandons en échange, c'est de savoir tout de suite ce qui doit partir et ce qui reste : c'est la seule information dont nous avons besoin pour aller vite." }
    ],
    erreurs: [
      { t: "Attendre le dernier jour pour appeler", d: "Une équipe et un véhicule se libèrent, une autorisation de voirie non. Chaque jour gagné sur l'appel élargit les solutions possibles." },
      { t: "Tout emballer sans étiqueter", d: "Le temps gagné au chargement se perd au triple à l'arrivée. Un mot par carton suffit, mais il faut ce mot." },
      { t: "Forcer une arrivée qui n'est pas prête", d: "Charger, stocker et livrer plus tard coûte moins qu'un double déménagement précipité, et supprime la pression sur la date." }
    ]
  },
...
/* Le composant unique. Une page l'appelle en une ligne : <FondPage cle="checklist" />.
   Si la cle n'existe pas, il ne rend rien, ce qui permet de brancher une page avant
   d'en avoir ecrit le contenu sans casser l'affichage. */
function FondPage({ cle, depart }) {
      ...
      <section className="sec">
        <div className="wrap">
          <div className="sec-head reveal">
            <div><div className="sec-num" style={{ fontFamily: "\"DM Sans\"" }}><span className="asterisk">*</span> {String(n0).padStart(2, '0')} / En détail</div></div>
            <h2 className="dim-em">{f.titre[0]} <em>{f.titre[1]}</em></h2>
          </div>
          <p className="lead" style={{ maxWidth: 820 }}>{f.intro}</p>
        </div>
      </section>
      {f.sections.map((s, i) => (
          ...
          <div className="wrap">
            <div className="sec-head reveal">
              <div><div className="sec-num" style={{ fontFamily: "\"DM Sans\"" }}>
                <span className="asterisk">*</span> {String(n0 + 1 + i).padStart(2, '0')} / Le détail</div></div>
              <h2 className="dim-em">{s.t}</h2>
            </div>
            <p style={{ maxWidth: 820, lineHeight: 1.78 }}>{s.d}</p>
          </div>
        </section>))}
      {f.erreurs &&
        <section className="sec">
          <div className="wrap">
            <div className="sec-head reveal">
              <div><div className="sec-num" style={{ fontFamily: "\"DM Sans\"" }}>
                <span className="asterisk">*</span> {String(n0 + 1 + f.sections.length).padStart(2, '0')} / À éviter</div></div>
              <h2 className="dim-em">Les trois erreurs <em>qu'on voit le plus souvent.</em></h2>
            </div>
...
Object.assign(window, { SERVICE_FOND, FondPage });
